	...
	public class Unit : MonoBehaviour {
		...
		public string GetDespStats(){ 
			...
			if(tower.type==_TowerType.Turret || tower.type==_TowerType.AOE){
				...
				if(currentDmgMax>0){
					if(currentDmgMin==currentDmgMax) text+="데미지:		 "+currentDmgMax.ToString("f0");
					else text+="데미지:		 "+currentDmgMin.ToString("f0")+"-"+currentDmgMax.ToString("f0");
				}
				
				float currentAOE=GetAOERadius();
				if(currentAOE>0) text+=" (범위공격)";
				...
				if(buff.damageBuff>0) text+="데미지 버프: "+((buff.damageBuff)*100).ToString("f0")+"%";
